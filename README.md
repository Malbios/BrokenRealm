...
The browser admin panel loads the behavior-module catalog and uses Monaco, loaded from a pinned CDN version with a textarea fallback, to edit modules in memory. It loads the server's authoritative scripting declarations and dependency sources for IntelliSense, preserves one model and undo history per module, and maps structured diagnostics to the correct module and source location. Check validates the complete affected graph without activation. Save recompiles the edited module and all transitive dependents, validates every registered class and affected object, and activates the graph atomically. Unsaved-change guards cover module and tab navigation. Any failure leaves the previous graph active. There is no authentication yet.
