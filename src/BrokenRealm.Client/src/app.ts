...
type PlayerUiStrings = {
  ...
  statusHunger: (value: number) => string;
  statusInventoryEmpty: string;
  statusInventoryGuest: string;
...
function tabSessionOwnerKey(sessionId: string): string {
  return `${TAB_SESSION_OWNER_PREFIX}${sessionId}`;
}

function claimTabSessionId(sessionId: string): boolean {
  try {
    const ownerKey = tabSessionOwnerKey(sessionId);
    const owner = localStorage.getItem(ownerKey);
    if (owner && owner !== tabInstanceId) return false;
...
function releaseTabSessionId(sessionId: string): void {
  try {
    const ownerKey = tabSessionOwnerKey(sessionId);
...
function initializeTabSessionId(): string {
...
function rememberTabSessionId(sessionId: string): void {
  if (!sessionId) return;
  if (sessionId !== tabSessionId) releaseTabSessionId(tabSessionId);
...
const statusHunger = document.querySelector<HTMLDivElement>("#status-hunger");
const statusInventory = document.querySelector<HTMLDivElement>("#status-inventory");
...
function formatInventorySummary(inventory: Record<string, number>): string {
...
function updatePlayerStatus(culture: Culture = selectedCulture()): void {
  if (!statusCharacter || !statusLocation || !statusHunger || !statusInventory) return;
